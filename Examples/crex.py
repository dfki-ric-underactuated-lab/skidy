...
s.Mb = []
s.Mb.append(s.MassMatrixMixedData(m1, s.InertiaMatrix(I1xx, I1xy, I1xz, I1yy, I1yz, I1zz),Matrix([c1x,c1y,c1z])))
s.Mb.append(s.MassMatrixMixedData(m2, s.InertiaMatrix(I2xx, I2xy, I2xz, I2yy, I2yz, I2zz),Matrix([c2x,c2y,c2z])))
s.Mb.append(s.MassMatrixMixedData(m3, s.InertiaMatrix(I3xx, I3xy, I3xz, I3yy, I3yz, I3zz),Matrix([c3x,c3y,c3z])))
s.Mb.append(s.MassMatrixMixedData(m4, s.InertiaMatrix(I4xx, I4xy, I4xz, I4yy, I4yz, I4zz),Matrix([c4x,c4y,c4z])))

q = Matrix([q1,q2,q3,q4])
qd = Matrix([dq1,dq2,dq3,dq4])
q2d = Matrix([ddq1,ddq2,ddq3,ddq4])

T = s.closed_form_kinematics_body_fixed_parallel(q,qd,q2d,simplify_expressions=0)

Q = s.closed_form_inv_dyn_body_fixed_parallel(q,qd,q2d,simplify_expressions=0)
